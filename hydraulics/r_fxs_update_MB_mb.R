...
##In the exponential function, the effect of changing Xo (the intercept) on kl is usually very small compared to that of any of the other parameters, so it is hard to fit with any kind of precision. This makes it hard to estimate error terms for Xo, and increases the error in the other parameters, since the model can't tell whether a given improvement in fit came from changing Xo or another parameter. I had very little success fitting Xo without NAs in the error terms, but the model fit really well when I removed Xo from the equation. I was concerned we wouldn't capture the tail of the function when kl is close to 0, but the graphs of the exponential fits look fine. I think this is a reasonable simplification and improves model fit, but if you think Xo is important, then you might have to fit it one curve at a time. 
...
do_the_thing_nonlinear <- function(input_df, model_type, pars1, par_lo1, par_hi1) {
  ...
  res<-anneal(model = model_type, par= pars1, source_data = input_df, var = var, par_lo=par_lo1, par_hi= par_hi1, dep_var = "kl", pdf = dnorm, max_iter=5000, show_display=F, temp_red=0.05)
  ...
  parvecLog<-c(Species= paste(input_df[1,1]), data.type=paste(input_df[1,2]),A = res$best_pars[1], B = res$best_pars[2], C = res$best_pars[3], D = res$best_pars[4],loglikeli = res$max_likeli, rsq = rsq, slope = slope, AIC = AIC, AICcorr = AICcorr, sterror_1 = sterror[1], sterror = sterror[2], sterror = sterror[3], sterror = sterror[4], N =N, gmax=gmax)
...
do_the_thing_linear <- function(input_df, model_type) {
  ...
  par_lo1= list(A=summary(fita)$coeff[1,1]*0.1, B =summary(fita)$coeff[2,1]*2, sd =0.005)
  par_hi1= list(A=summary(fita)$coeff[1,1]*2, B =summary(fita)$coeff[2,1]*0.1, sd =20)
  #########Update from Megan: I decreased the sd of the par_lo1, linear fits now match the lm results
  
  res<-anneal(model = model_type, par= pars, source_data = input_df, var = var, par_lo=par_lo1, par_hi= par_hi1, dep_var = "kl", pdf = dnorm, max_iter=5000, show_display=F)
